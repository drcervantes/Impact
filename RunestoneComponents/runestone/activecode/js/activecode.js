...
ActiveCode.prototype.buildProg = function() {
    ...
    if (this.grid) {
        var reg = /import graphics/gi;

        prog = prog.replace(reg, "import turtle; " +
                                 "t = turtle.Turtle(); " + 
                                 "w = turtle.Screen(); " +
                                 "t.ht(); " + 
                                 "adj = t.window_width() / 2;");

        reg = /(\t*)dot\((.*)\)/gi;
        prog.search(reg);
        var dotParams = (RegExp.$2).split(',');
        
        if (dotParams.length > 2) {
            prog = prog.replace(reg, "$1t.dot((" + dotParams[0] + "*2)-adj," + dotParams[1] + "*2,\"" + dotParams[2] +"\",3);");
        }
        else {
            prog = prog.replace(reg, "$1c = t.color();t.dot((" + dotParams[0] + "*2)-adj," + dotParams[1] + "*2,c,3);");
        }

        reg = /clear\(\)/gi;
        prog = prog.replace(reg, "w.reset();t.grid();t.ht();");
    }
...
ActiveCode.prototype.runProg = function () {
    ...
    if (this.grid && this.autorun) {
        this.editor.setValue("import graphics\n\n");
        this.autorun = false;
    }

    if ($(this.origElem).data("preserve")) {
        Sk.TurtleGraphics.preserve = true;
    }
